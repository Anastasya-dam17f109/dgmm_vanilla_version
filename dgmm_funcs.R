...
dgmm_grid_search <- function(data0, k, it, eps,init, init_est,seed = 1,type='BIC'){
  ##############################################################################
  #
  #                Full Grid
  #                    Search
  ...
  for (i in 1:nrow(r1)){
    # print(i)
    p1 <- r1[i,1]
    dgmm1 <- deepgmm(data0, layers=1, k=k, r=p1, seed = seed,it=it,eps=eps,init=init, init_est=init_est)
...
dgmm_param_extraction <- function(data, layers, k, r,it,eps,seed,init, init_est,target_id){ 
    ...
    return(list(all_w,all_mu,all_var, tot.k))
}
